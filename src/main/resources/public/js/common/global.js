...
function createSubGrid(gridTable,gridPager,objList,n,lastRowId,subExpandedCallback,subColapsedCallback){
	var isFirst = true;
	var obj = objList[n];
	var url = obj.url;
	...
	$("#"+gridTable).jqGrid({
		...
		subGridRowExpanded: function (subgridDivId, rowId) {
			...
			if(!isnull(nextObj)){
				...
				url += "?id="+rowId;
				nextObj.url = url;
				createSubGrid(subgridTableId,subgridPagerId,objList,n2,lastRowId,subExpandedCallback,subColapsedCallback);
			}
		...
		subGridRowColapsed: function (subgridDivId, rowId) {
			if(!isnull(subColapsedCallback)){
				subColapsedCallback(n);
			}
		},
		url:url,
		mtype: 'POST',
		 ...
		 gridComplete : function() {
				if(isFirst){
					...
					$(window).on('resize.jqGrid', function () {
				    	width = $("#gview_"+gridTable).parent().parent().width();
				    	if(!isnull(width)){
				    		$("#"+gridTable).jqGrid( 'setGridWidth', width);
				    		$("#gview_"+gridTable+" .ui-jqgrid-bdiv").css("width",width+1);
				    	}
				    });
